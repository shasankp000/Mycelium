...
def test_expert_workflow(test_sentences):
    ...
    for i, sentence in enumerate(test_sentences, 1):
        ...
        if decision_flag == 'use_existing_expert':
            ...
            if conf_diff < 0.1:
                ...
                conf_alignment = "⚠️ Moderate difference"
            else:
            ...
            print(f"\n   ℹ️ Decision is '{decision_flag}' - no model inference needed")
            results.append({
                'sentence': sentence,
                'tags': tags,
                'decision': decision_flag,
    ...
    if use_existing_cases:
        ...
        mismatches = sum(1 for r in use_existing_cases if r.get('alignment') == '⚠️ MISMATCH')
