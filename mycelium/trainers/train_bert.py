...
class DomainBERTTrainer:
    ...
    def __init__(
        ...
        print(f"\U0001f527 Domain       : {domain}")
        print(f"\U0001f527 Device       : {self.device}")
        print(f"\U0001f4e6 Base model   : {model_name}")
        print(f"\U0001f3f7\ufe0f  Labels       : {self.meta['positive_label']} \u2192 1 "
    ...
    def create_data_loaders(
        ...
        print(
            f"\n\U0001f504 Creating data loaders "
    ...
    def initialize_model(self, num_labels: int = 2):
        print(f"\n\U0001f916 Initialising BERT ({num_labels} classes)...")
    ...
    def train(
        ...
        print(f"\n\U0001f680 Training for {epochs} epoch(s) | lr={learning_rate}")
        ...
        for epoch in range(epochs):
            ...
            print(f"\U0001f4ca Train  \u2014 loss: {train_loss:.4f}  acc: {train_acc:.4f}")

            val_loss, val_acc, val_prec, val_rec, val_f1, _, _ = self._evaluate(
                val_loader, "Validation"
            )
            print(
                f"\U0001f4ca Val    \u2014 loss: {val_loss:.4f}  acc: {val_acc:.4f}  "
            ...
            if val_f1 > best_f1:
                ...
                print(f"\U0001f4be Best model saved (F1: {val_f1:.4f}) \u2192 {save_path}")

        hist_path = save_path / "training_history.json"
        with open(hist_path, "w") as f:
            json.dump(history, f, indent=2)
        print(f"\n\U0001f4be Training history \u2192 {hist_path}")
        return history
    ...
    def test(self, test_loader):
        if test_loader is None:
            ...
            return None
        print(f"\n{'='*60}")
        print("\U0001f9ea Final test evaluation")
        print("=" * 60)

        loss, acc, prec, rec, f1, preds, labels = self._evaluate(test_loader, "Test")
        print(
            f"\n\U0001f4ca Test \u2014 acc: {acc:.4f}  P: {prec:.4f}  "
            f"R: {rec:.4f}  F1: {f1:.4f}"
        )
        target_names = [
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--train_data", default=_cfg_get("train_data", None), help="Path to train CSV.")
    p.add_argument("--val_data",   default=_cfg_get("val_data",   None), help="Path to validation CSV.")
    p.add_argument("--test_data",  default=_cfg_get("test_data",  None), help="Path to test CSV (optional).")
    p.add_argument("--output_dir", default=_cfg_get("output_dir", None), help="Directory to save the trained model.")
    p.add_argument("--model_name", default=_cfg_get("model_name", "bert-base-uncased"), help="HuggingFace model identifier.")
    p.add_argument("--epochs",         type=int,   default=_cfg_get("epochs",         3),    help="Number of training epochs.")
    p.add_argument("--batch_size",     type=int,   default=_cfg_get("batch_size",     16),   help="Batch size.")
    p.add_argument("--max_length",     type=int,   default=_cfg_get("max_length",     128),  help="Max token sequence length.")
    p.add_argument("--learning_rate",  type=float, default=_cfg_get("learning_rate",  2e-5), help="AdamW learning rate.")
    p.add_argument(
        "--label_col",
        default=_cfg_get("label_col", None),
        help=f"CSV column that holds the label strings. Auto-detected from {_LABEL_COL_CANDIDATES} if not set.",
    )
    p.add_argument(
        "--text_col",
        default=_cfg_get("text_col", None),
        help=f"CSV column that holds the text/sentence. Auto-detected from {_TEXT_COL_CANDIDATES} if not set.",
    )
    return p
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"\U0001f389 Done! Model saved to: {output_dir}")
    print(f"{'='*60}\n")
