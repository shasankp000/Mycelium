...
def _resolve_env(default: Any, env_var: Optional[str], section: str, key: str) -> Optional[Any]:
    if env_var:
...
def superposition_variance_threshold() -> float:
    return _get("routing", "superposition_variance_threshold", 0.08, "SUPERPOSITION_VARIANCE_THRESHOLD")
...
def api_allowed_origins() -> List[str]:
    return get_list("api", "allowed_origins", ["http://localhost:3000", "http://127.0.0.1:3000"])
