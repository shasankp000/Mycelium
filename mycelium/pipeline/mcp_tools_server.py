...
def _web_search(query: str) -> Dict[str, Any]:
    ...
    if DDGS is not None:
        ...
        for attempt in range(1, _DDGS_MAX_RETRIES + 1):
            try:
                ...
                last_exc = exc
                wait = 2 ** (attempt - 1)
                logger.warning("_web_search ddgs attempt %d/%d failed (%s) — retrying in %ds", attempt, _DDGS_MAX_RETRIES, exc, wait)
                time.sleep(wait)
    ...
    try:
        ...
        if data.get("AbstractText"):
            results_list.append({"title": data.get("Heading", ""), "snippet": data["AbstractText"], "url": data.get("AbstractURL", "")})
        for t in data.get("RelatedTopics", [])[:4]:
            if isinstance(t, dict) and t.get("Text"):
                results_list.append({"title": t.get("Text", "")[:80], "snippet": t.get("Text", ""), "url": t.get("FirstURL", "")})
        if results_list:
    ...
    try:
        ...
        results_list = [
            {"title": re.sub(r"<[^>]+>", "", t).strip(), "snippet": re.sub(r"<[^>]+>", "", s).strip()}
    ...
    return {"results": [], "status": "empty", "note": "All web search backends returned no results for this query.", "source": "web_search"}


def _academic_search(query: str) -> Dict[str, Any]:
    try:
        url = "https://api.semanticscholar.org/graph/v1/paper/search"
        params = {"query": query, "limit": 4, "fields": "title,authors,year,abstract,externalIds"}
...
def _knowledge_base(query: str) -> Dict[str, Any]:
    try:
        resp = _SESSION.get(
            ...
            params={"action": "wbsearchentities", "search": query, "language": "en", "limit": 5, "format": "json", "type": "item"},
        ...
        data = resp.json()
        entities = [
            {"id": item.get("id", ""), "label": item.get("label", ""), "description": item.get("description", ""), "url": item.get("url", "")}
...
async def list_tools() -> list[mcp_types.Tool]:
    return [
        ...
        mcp_types.Tool(
            ...
            inputSchema={"type": "object", "properties": {"query": {"type": "string", "description": "Academic search query"}}, "required": ["query"]},
        ),
        mcp_types.Tool(
            name="knowledge_base",
            description="Structured entity facts via Wikidata. Use for entity definitions, taxonomy, and factual attributes.",
            inputSchema={"type": "object", "properties": {"query": {"type": "string", "description": "Entity name to look up"}}, "required": ["query"]},
        ),
        mcp_types.Tool(
            name="calculator",
            description="Safe arithmetic evaluator. Use for numeric expressions, unit conversions, and mathematical computations only.",
            inputSchema={"type": "object", "properties": {"expression": {"type": "string", "description": "A Python-compatible math expression, e.g. 'sqrt(2) * pi'"}}, "required": ["expression"]},
        ),
    ]
