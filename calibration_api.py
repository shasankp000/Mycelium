...
# ---------------------------------------------------------------------------
# In-memory job store.
# Replace with Redis or a proper DB for multi-worker / multi-process setups.
# ---------------------------------------------------------------------------
_jobs: Dict[str, dict] = {}
...
def _blocking_calibration(job_id: str, app_state) -> None:
    ...
    try:
        ...
        # Inline import keeps startup fast when the router is included but
        # calibration has not been requested yet.
        from unified_expert_system import UnifiedExpertSystem  # noqa: PLC0415
        ...
        # Persist the ready system on app.state so query endpoints can use it.
...
async def _run_calibration(job_id: str, app_state) -> None:
    """Async wrapper: delegates blocking work to a thread-pool executor."""
...
async def start_calibration(
    ...
    """Kick off expert calibration as a background job.

    Returns immediately with a ``job_id``.  Poll
    ``GET /api/calibrate/status/{job_id}`` to track progress.
    """
    job_id = _new_job()
...
async def calibration_status(job_id: str) -> JSONResponse:
    """Return the current state of a calibration job.

    ``status`` values:
    - ``"pending"``  — queued, not yet started
    - ``"running"``  — in progress
    - ``"complete"`` — finished successfully; expert system is ready
    - ``"error"``    — failed; ``error`` field contains the exception message
...
async def calibration_ready(request: Request) -> JSONResponse:
    """Return whether a fully-initialised expert system is available.

    Useful for health-check endpoints or frontend startup guards.
    """
